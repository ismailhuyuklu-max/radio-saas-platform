...
function typeLabel(it: MediaLibraryItem): string {
  return it.kind === 'sponsor' ? 'Reklam' : (PART_LABELS[it.part_code as keyof typeof PART_LABELS] ?? it.part_code);
}
  ...
  <div class="ml">
    <header class="ml__head">
      <div>
        ...
        <p class="ml__sub">Haber, spor, ekonomi, hava ve reklam ses dosyalarını dinleyin · {{ items.length }} kayıt</p>
      </div>
    </header>
...
.ml {
  ...
  gap: var(--sp-4);
  padding-bottom: 96px; /* room for the sticky player */
}
.ml__head h1 {
  margin: 0;
